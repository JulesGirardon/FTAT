...
if(isset($_POST['user'],$_POST['role'],$_POST['equipe']))  //Recuperation des infos
{
    ...
    if (isset($bdd)) 
    {
        try 
        {
            ...
            {
                $sql = 'UPDATE ftat.rolesutilisateurprojet
                        JOIN ftat.projets ON projets.IdP = rolesutilisateurprojet.IdP
                        SET rolesutilisateurprojet.IdR = 0
                        WHERE projets.IdEq = :IdEq';

                $changeRole = $bdd->prepare($sql);
                $changeRole->bindParam(":IdEq", $IdEq);
                $changeRole->execute();
            }

            //Ajoute à l'equipe le nouveau membre
            $sql = 'INSERT INTO ftat.rolesutilisateurprojet (rolesutilisateurprojet.IdU, rolesutilisateurprojet.IdR, rolesutilisateurprojet.IdP)
                    VALUES (:IdU,:IdR,:IdP)';
            $addUser = $bdd->prepare($sql);
